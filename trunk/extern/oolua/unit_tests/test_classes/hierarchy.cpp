...
namespace BASE_HELPERS
{
	...
	struct AddName
	{
		void add(std::vector<std::string>& v)
		{
			...
			AddName<TL,Index+1,typename TYPELIST::at_with_default<TL,Index+1,TYPE::Null_type>::type > adder;
			adder.add(v);
		}
	};
	...
	struct AddBaseNamesToVector
	{
		void add(std::vector<std::string> & v)
		{
			AddName<AllBases,0,typename TYPELIST::at_with_default<AllBases,0,TYPE::Null_type>::type > adder;
			adder.add(v);
		}
	};
...
public:
	...
	{
		static const int result_length(mpl::size<OOLUA::Proxy_class<DerivedFromTwoAbstractBasesAndAbstract3>::AllBases>::value);
		BOOST_STATIC_ASSERT(result_length == 4);
		CPPUNIT_ASSERT_EQUAL(int(4),result_length);
	}
	BOOST_STATIC_ASSERT((mpl::size<OOLUA::Proxy_class<DerivedFromTwoAbstractBasesAndAbstract3>::Bases>::value == 2));
	void rootBases_DerivedFromTwoAbstractBasesAndAbstract3_lengthEqualsThree()
	{
		static const int result_length(mpl::size<OOLUA::INTERNAL::FindRootBases<DerivedFromTwoAbstractBasesAndAbstract3>::Result>::value);
		BOOST_STATIC_ASSERT(result_length == 3);
