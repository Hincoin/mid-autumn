...
//////////////////////////////////////////////////////////////////////////
HPHAllocator::HPHAllocator() : mMRFreeBlock(0) {
	...
	debug_alloc_size = 0;
	debug_found_count = 0;
	debug_total_allocated = 0;
	debug_insert_count = 0;
}

HPHAllocator::~HPHAllocator() {
	purge();
...
HPHAllocator::page* HPHAllocator::bucket::get_free_page() {
	if (!mPageList.empty()) {
...
void* HPHAllocator::bucket::alloc(page* p) {
...
void HPHAllocator::bucket::free(page* p, void* ptr) {
...
void* HPHAllocator::bucket_system_alloc() {
...
void HPHAllocator::bucket_system_free(void* ptr) {
	assert(ptr);
#ifdef _WIN32
	virtual_free(ptr);
#else
...
void* HPHAllocator::bucket_alloc(size_t size) {
...
void* HPHAllocator::bucket_alloc_direct(unsigned bi) {
	assert(bi < NUM_BUCKETS);
#ifdef MULTITHREADED
	mutex::scoped lock(mBuckets[bi].get_lock());
#endif
...
void* HPHAllocator::bucket_realloc(void* ptr, size_t size) {
...
void HPHAllocator::bucket_free(void* ptr) {
	page* p = ptr_get_page(ptr);
...
void HPHAllocator::bucket_free_direct(void* ptr, unsigned bi) {
	assert(bi < NUM_BUCKETS);
	page* p = ptr_get_page(ptr);
...
void HPHAllocator::bucket_purge() {
	for (unsigned i = 0; i < NUM_BUCKETS; i++) {
#ifdef MULTITHREADED
...
void HPHAllocator::split_block(block_header* bl, size_t size) {
...
HPHAllocator::block_header* HPHAllocator::shift_block(block_header* bl, size_t offs) {
	assert(offs > 0);
...
HPHAllocator::block_header* HPHAllocator::coalesce_block(block_header* bl) {
	assert(!bl->used());
...
void* HPHAllocator::tree_system_alloc(size_t size) {
...
void HPHAllocator::tree_system_free(void* ptr, size_t size) {
	assert(ptr);
	assert(size/PAGE_SIZE*PAGE_SIZE == size);
#ifdef _WIN32
	virtual_free(ptr);
...
HPHAllocator::block_header* HPHAllocator::tree_add_block(void* mem, size_t size) {
...
HPHAllocator::block_header* HPHAllocator::tree_grow(size_t size) {
...
HPHAllocator::block_header* HPHAllocator::tree_extract(size_t size) {
	...
	if (bestNode == mFreeTree.end())
		return NULL;
	else debug_found_count ++;
	block_header* cur_block = bestNode->get_block();
...
HPHAllocator::block_header* HPHAllocator::tree_extract_aligned(size_t size, size_t alignment) {
	block_header* bestBlock = mMRFreeBlock;
	if (bestBlock) {
...
void HPHAllocator::tree_attach(block_header* bl) {
	if (mMRFreeBlock) {
...
void HPHAllocator::tree_detach(block_header* bl) {
	if (mMRFreeBlock == bl) {
		mMRFreeBlock = NULL;
		return;
	}
...
void* HPHAllocator::tree_alloc(size_t size) {
#ifdef MULTITHREADED
	mutex::scoped lock(mTreeMutex);
#endif
...
void* HPHAllocator::tree_alloc_aligned(size_t size, size_t alignment) {
#ifdef MULTITHREADED
	mutex::scoped lock(mTreeMutex);
#endif
	if (size < sizeof(free_node))
...
void* HPHAllocator::tree_realloc(void* ptr, size_t size) {
#ifdef MULTITHREADED
	mutex::scoped lock(mTreeMutex);
#endif
	if (size < sizeof(free_node))
...
void* HPHAllocator::tree_realloc_aligned(void* ptr, size_t size, size_t alignment) {
...
size_t HPHAllocator::tree_resize(void* ptr, size_t size) {
#ifdef MULTITHREADED
	mutex::scoped lock(mTreeMutex);
#endif
	if (size < sizeof(free_node))
...
void HPHAllocator::tree_free(void* ptr) {
#ifdef MULTITHREADED
	mutex::scoped lock(mTreeMutex);
#endif
	block_header* bl = ptr_get_block_header(ptr);
...
void HPHAllocator::tree_purge_block(block_header* bl) {
	assert(!bl->used());
...
#ifdef DEBUG_ALLOCATOR
void HPHAllocator::debug_record::record_stack() {
...
void HPHAllocator::debug_record::write_guard() {
...
bool HPHAllocator::debug_record::check_guard() const {
...
void HPHAllocator::debug_record_map::initial_fill(void* ptr, size_t size) {
...
bool HPHAllocator::debug_record_map::add(void* ptr, size_t size, debug_source source) {
...
HPHAllocator::debug_info HPHAllocator::debug_record_map::remove(void* ptr) {
...
HPHAllocator::debug_info HPHAllocator::debug_record_map::remove(void* ptr, size_t size) {
	iterator it = this->find(ptr);
	// if this asserts most likely the pointer was already deleted
	// or the pointer points to a static or a global variable.
	assert(it != this->end());
...
HPHAllocator::debug_info HPHAllocator::debug_record_map::replace(void* ptr, void* newPtr, size_t size, debug_source source) {
	iterator it = this->find(ptr);
	// if this asserts most likely the pointer was already deleted
	assert(it != this->end());
	debug_record* record = it;
...
HPHAllocator::debug_info HPHAllocator::debug_record_map::update(void* ptr, size_t size) {
	iterator it = this->find(ptr);
	// if this asserts most likely the pointer was already deleted
	assert(it != this->end());
	debug_record* record = it;
...
void HPHAllocator::debug_record_map::check(void* ptr) const {
...
void HPHAllocator::debug_record_map::purge() {
	mRecords.purge();
}

void* HPHAllocator::debug_add(void* ptr, size_t size, debug_source source) {
	if (!ptr)
		return NULL;
	{
#ifdef MULTITHREADED
...
void HPHAllocator::debug_remove(void* ptr) {
#ifdef MULTITHREADED
	mutex::scoped lock(mDebugMutex);
#endif
...
void HPHAllocator::debug_remove(void* ptr, size_t size) {
#ifdef MULTITHREADED
	mutex::scoped lock(mDebugMutex);
#endif
...
void HPHAllocator::debug_replace(void* ptr, void* newPtr, size_t size, debug_source source) {
	if (!newPtr)
		return;
#ifdef MULTITHREADED
	mutex::scoped lock(mDebugMutex);
...
void HPHAllocator::debug_update(void* ptr, size_t size) {
#ifdef MULTITHREADED
	mutex::scoped lock(mDebugMutex);
#endif
	assert(size <= this->size(ptr));
...
void HPHAllocator::debug_check(void* ptr) {
	mDebugMap.check(ptr);
}

void HPHAllocator::debug_purge() {
#ifdef MULTITHREADED
	mutex::scoped lock(mDebugMutex);
#endif
	mDebugMap.purge();
