	...
	void table_containing_key_value_pair(OOLUA::Script * lvm,OOLUA::Lua_table* t,std::string const& key,T& value)
	{
		lvm->run_chunk(
			std::string("func = function() ")
				+std::string("t = {[\"func\"] = function(x) print(x) end,} ")
		...
		lua_State* ls =(lua_State*) lvm->get_ptr(); 
		OOLUA::push2lua(ls,"func");//table key
		lua_gettable(ls, -2);//table value
		bool is_func = lua_type(ls,-1) == LUA_TFUNCTION;
		OOLUA::push2lua(ls,"lualua lua");
