...
#pragma omp parallel for
			for (int y = miny; y < maxy; y += q)
			{
				...
				/// skip the line that x_left[i] > x_right[i]
				float x_right[q];
				float x_left[q];
				...
				for (int x = minx; x < maxx; x += q)
				{
					...
					//computed only once for this line
					if (! computed_flag)
					{
						if (is_middle_left)
						{
							for (int k = 0;k < q; ++k)
							{
								if (y+k < tri_v2->y) // betwenn v1,v2
								{
									...
									float lx = round(lerp(tri_v1->x,tri_v2->x,left_ty[k]));
									if (lx >= maxx)
									{x_left[k] = lx+1;x_right[k] = lx-1;continue;}
									right_ty[k] = linearStep(tri_v1->y,tri_v3->y,(float)(y+k));
									float rx = round(lerp(tri_v1->x,tri_v3->x,right_ty[k]));
									if (rx < minx)
									{x_left[k] = rx+1;x_right[k] = rx-1;continue;}
								...
								else
								{
									...
									float lx = round(lerp(tri_v2->x,tri_v3->x,left_ty[k]));
									if (lx >= maxx)
									{x_left[k] = lx+1;x_right[k] = lx-1;continue;}
									right_ty[k] = linearStep(tri_v1->y,tri_v3->y,(float)(y+k));
									float rx = round(lerp(tri_v1->x,tri_v3->x,right_ty[k]));
						...
						else
						{
							for (int k = 0;k < q; ++k)
							{
								if (y+k < tri_v2->y) // betwenn v1,v2
								{
									...
									float lx = round(lerp(tri_v1->x,tri_v3->x,left_ty[k]));
									if (lx >= maxx)
									{x_left[k] = lx+1;x_right[k] = lx-1;continue;}
									right_ty[k] = linearStep(tri_v1->y,tri_v2->y,(float)(y+k));
									float rx = round(lerp(tri_v1->x,tri_v2->x,right_ty[k]));
									if (rx < minx)
									{x_left[k] = rx+1;x_right[k] = rx-1;continue;}
									//same pixel ?
									//x_step[k] = rx - lx >= 1.f ? (1.f)/(rx-lx):0;
								...
								else
								{
									left_ty[k] = linearStep(tri_v1->y,tri_v3->y,(float)(y+k));
									float lx = round(lerp(tri_v1->x,tri_v3->x,left_ty[k]));
									if (lx >= maxx){x_left[k] = lx+1;x_right[k] = lx-1;continue;}
									right_ty[k] = linearStep(tri_v2->y,tri_v3->y,(float)(y+k));
									float rx = round(lerp(tri_v2->x,tri_v3->x,right_ty[k]));
									if (rx < minx)
									{x_left[k] = rx+1;x_right[k] = rx-1;continue;}
					...
					if (a == 0xF && b == 0xF && c == 0xF)
					{
						...
						{
							float x_step = x_right[iy] - x_left[iy];
							//same pixel or invalid line
							if (x_step <1.f)
							{
								continue;
							}
							x_step = 1.f/x_step;
							x_fraction = (x-x_left[iy]) * x_step;/// x_fraction (- [0,1]

							z_start = lerp(z_left[iy],z_right[iy],x_fraction);
							z_step= x_step *(z_right[iy] - z_left[iy]);
							...
							DUFFS_DEVICE(8,int,MARasterizerBase::Vertex::linear_var_cnt,
								...
								(linear_varyings_right[iy][i_] - linear_varyings_left[iy][i_] )* x_step;
							++i_;
							);
					...
					else // Partially covered block
					{
						...
						for (int iy = 0; iy <  q; iy++)
						{
							...
							{
								x_step = 1.f/x_step;
								x_fraction = (xl-x_left[iy]) * x_step;/// x_fraction (- [0,1]
                ...
                for (int x = minx; x < maxx; x += q)
                {
                    ...
                    if (a == 0xF && b == 0xF && c == 0xF)
                    {
                        ...
                        for (int iy = 0; iy < q; iy++)
                        {
                            ...
                            inv_w_step = x_step * (inv_w_right[iy] - inv_w_left[iy]);
                            ...
                            DUFFS_DEVICE(8,int,MARasterizerBase::Vertex::persp_var_cnt,
                                         persp_varyings_step[i_]=
                                             (persp_varyings_right[iy][i_] - persp_varyings_left[iy][i_])*x_step;
                                         ++i_;
                                        );
                            ///
                            for (int ix = x; ix < x + q; ix++)
